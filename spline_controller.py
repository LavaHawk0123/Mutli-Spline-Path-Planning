...
class CubicBezierCurve:
    ...
    def setCoeff(self,t):
        self.P0_coeff = -(t**3)+3*(t**2)-3*t+1
        self.P1_coeff = 3*(t**3)-6*(t**2)+3*t
        self.P2_coeff = -3*(t**3)+3*(t**2)
        self.P3_coeff = t**3
...
def getControlPoints(viapoints,theta1,theta2):
    # Theta1 - initial heading of the robot
    # Theta 2 - final heading of the robot
    # viapoints - list of viapoints from vernoi diagram
    global_c1 = Coordinate(0,0)
    global_c2 = Coordinate(0,0)
    alpha1 = np.linalg.norm(np.array(viapoints[1])-np.array(viapoints[0]))
    alpha2 = np.linalg.norm(np.array(viapoints[len(viapoints)-1])-np.array(viapoints[len(viapoints)-2]))
    global_c1.x = viapoints[0][0] + 0.5*np.cos(theta1*np.pi/180)
    global_c1.y = viapoints[0][1] + 0.5*np.sin(theta1*np.pi/180)
    global_c2.x = viapoints[len(viapoints)-1][0] + alpha1*np.cos(theta2*np.pi/180)
    global_c2.y = viapoints[len(viapoints)-1][1] + alpha2*np.sin(theta2*np.pi/180)
    print("Global c1 : ",global_c1.x," , ",global_c1.y)
    print("Global c2 : ",global_c2.x," , ",global_c2.y)
    return global_c1,global_c2
    
        
if __name__ == "__main__":
    rospy.init_node('spline_controller')
    viapoints=[(0,0),(3,0),(3,5),(8,5),(8,15)]
    c1,c2 = getControlPoints(viapoints,30, -45)
    path = findPath(viapoints,c1,c2)
